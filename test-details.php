...
$query=mysqli_query($con,"update tbltestrecord set Reportstat='$status',Asstoname='$aname',AsstoempId='$pid',Asstime='$assigntime' where Slno='$testid'");
...
$query=mysqli_query($con,"insert into tblreporttracking(Ordno,Status,Remark,RemarkBy) values('$orderid','$status','$remark','$rby')");
$query2=mysqli_query($con,"update tbltestrecord set Reportstat='$status',Finreport='$newreportfile',Reportuptime='$uploadtime' where Ordno='$orderid'");
...
$query=mysqli_query($con,"insert into tblreporttracking(Ordno,Status,Remark,RemarkBy) values('$orderid','$status','$remark','$rby')");
$query2=mysqli_query($con,"update tbltestrecord set Reportstat='$status' where Ordno='$orderid'");
echo '<script>alert("Status updated successfully")</script>';
echo "<script>window.location.href='all-test.php'</script>";
endif;  
...
join tblpatients on tblpatients.Phno=tbltestrecord.Patientphno
where  tbltestrecord.Slno='$testid'");
...
<!-- personal information --->
 ...
 <table class="table table-bordered"  width="100%" cellspacing="0">
    ...
    <td><?php echo $row['GovtId'];?></td>
    </tr>


     <tr>
    <th>Govt Issued Id No</th> 
    <td><?php echo $row['GovtIdno'];?></td>
    </tr>


     <tr>
    <th>Full Address</th> 
    <td><?php echo $row['Fulladdrs'];?></td>
    </tr>
...
<?php if($row['AsstoempId']!=''):?>
  <tr>
    <th>Assign To</th> 
    <td><?php echo $row['Asstoname'];?>-(<?php echo $row['AsstoempId'];?>)</td>
    </tr>

    <tr>
    <th>Assigned Date</th> 
    <td><?php echo $row['Asstime'];?></td>
    </tr>
<?php endif;?>
<?php if($row['Finreport']!=''):?>
  <tr>
    <th>Report</th> 
    <td><a href="reportfiles/<?php echo $row['Finreport'];?>" target="_blank">Download</a></td>
    </tr>

    <tr>
    <th>Report Delivered Time</th> 
    <td><?php echo $row['Reportuptime'];?></td>
    </tr>
<?php endif;?>
...
where tblreporttracking.Ordno='$orderid'");
...
<?php while($result=mysqli_fetch_array($ret)){?>
    ...
    <td><?php echo $result['Posttime'];?></td>
    <td><?php echo $result['Adname'];?></td>
    </tr>
...
<form method="post">
          <p>  <select class="form-control" name="assignto" required="true">
            ...
            <?php $sql=mysqli_query($con,"select FN,EmpID from tblphlebotomist");
            while ($result=mysqli_fetch_array($sql)) {
            ?>
            <option value="<?php echo $result['FN']."-".$result['EmpID'];?>"><?php echo $result['FN'];?>-(<?php echo $result['EmpID'];?>)</option>
        <?php } ?>
            </select></p>
            <p>
...
<form method="post" enctype="multipart/form-data" >
  ...
  $query1=mysqli_query($con,"select Reportstat from tbltestrecord where Ordno='$orderid'");
  while($result=mysqli_fetch_array($query1)):
$reportstatus=$result['Reportstat'];
endwhile;
  ?>
